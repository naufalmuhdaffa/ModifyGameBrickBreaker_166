...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.canvas.bind('<Left>', self.on_key_press)
        self.canvas.bind('<Right>', self.on_key_press)
    ...
    def update_lives_text(self):
        ...
        if self.hud_lives is None:
            self.hud_lives = self.draw_text(50, 20, text, 15)
        else:
            self.canvas.itemconfig(self.hud_lives, text=text)

    def update_score_text(self):
        text = f'Score: {self.score}'
        if self.hud_score is None:
            self.hud_score = self.draw_text(self.width - 50, 20, text, 15)
        else:
